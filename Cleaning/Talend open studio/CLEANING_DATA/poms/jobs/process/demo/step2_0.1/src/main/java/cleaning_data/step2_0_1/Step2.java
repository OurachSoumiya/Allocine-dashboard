...
package cleaning_data.step2_0_1;
 ...
 * @status
 */
public class Step2 implements TalendJob {
	...
	private final String jobName = "Step2";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Step2.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_CLEANING_DATA_Step2 = new byte[0];
		static byte[] commonByteArray_CLEANING_DATA_Step2 = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_CLEANING_DATA_Step2.length) {
					if (length < 1024 && commonByteArray_CLEANING_DATA_Step2.length == 0) {
						commonByteArray_CLEANING_DATA_Step2 = new byte[1024];
					} else {
						commonByteArray_CLEANING_DATA_Step2 = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_CLEANING_DATA_Step2, 0, length);
				strReturn = new String(commonByteArray_CLEANING_DATA_Step2, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_CLEANING_DATA_Step2, 0, length);
				strReturn = new String(commonByteArray_CLEANING_DATA_Step2, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_CLEANING_DATA_Step2) {
	...
	public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_1.addRow(new String[] { "movieName", "director", "Actor1", "Actor2", "Actor3", "Hour",
						"minutes", "type1", "type2", "type3", "AllowedAge", "Synopsis", "SpectatorsRating",
						"PressRating", });
...
///////////////////////    			
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:LXhN4Idco4usHwmSsyO/erPeGD/e/Rw8cx8ppUn4u5fU5ISv");
...
///////////////////////		
						...
						if (row1.movieName != null) { //
							row_tLogRow_1[0] = String.valueOf(row1.movieName);

						} //

						if (row1.director != null) { //
							row_tLogRow_1[1] = String.valueOf(row1.director);

						} //

						if (row1.Actor1 != null) { //
							row_tLogRow_1[2] = String.valueOf(row1.Actor1);

						} //

						if (row1.Actor2 != null) { //
							row_tLogRow_1[3] = String.valueOf(row1.Actor2);

						} //

						if (row1.Actor3 != null) { //
							row_tLogRow_1[4] = String.valueOf(row1.Actor3);

						} //

						if (row1.Hour != null) { //
							row_tLogRow_1[5] = String.valueOf(row1.Hour);

						} //

						if (row1.minutes != null) { //
							row_tLogRow_1[6] = String.valueOf(row1.minutes);

						} //

						if (row1.type1 != null) { //
							row_tLogRow_1[7] = String.valueOf(row1.type1);

						} //

						if (row1.type2 != null) { //
							row_tLogRow_1[8] = String.valueOf(row1.type2);

						} //

						if (row1.type3 != null) { //
							row_tLogRow_1[9] = String.valueOf(row1.type3);

						} //

						if (row1.AllowedAge != null) { //
							row_tLogRow_1[10] = String.valueOf(row1.AllowedAge);

						} //

						if (row1.Synopsis != null) { //
							row_tLogRow_1[11] = String.valueOf(row1.Synopsis);

						} //

						if (row1.SpectatorsRating != null) { //
							row_tLogRow_1[12] = String.valueOf(row1.SpectatorsRating);

						} //

						if (row1.PressRating != null) { //
							row_tLogRow_1[13] = String.valueOf(row1.PressRating);
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final Step2 Step2Class = new Step2();

		int exitCode = Step2Class.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Step2.class.getClassLoader()
					.getResourceAsStream("cleaning_data/step2_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Step2.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Step2");
		}
...
/************************************************************************************************
 * 56559 characters generated by Talend Open Studio for Data Integration on the
 * 18 mars 2024 à 10:20:01 GMT
 ************************************************************************************************/